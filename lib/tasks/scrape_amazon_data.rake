namespace :amazon do
  ...
  task :scrape_data => :environment do
    ...
    product_data.each do |product|
      Product.find_or_create_by(title: product[:title], img_url: product[:img_url], price: product[:price], customer_review: product[:customer_review], review_count: product[:review_count], asin: product[:asin])
    end
  end
end
